...
function handleWebSocketMessage(data) {
    switch (data.type) {
        ...
        case 'ai_update':
            // Ensure ai_update triggers full UI update and pending orders
            if (data.data && window.updateAIDisplay) {
                window.updateAIDisplay(data.data);
                // Also update pending orders if present
                if (data.data.pending_orders && window.updatePendingOrdersDisplay) {
                    window.updatePendingOrdersDisplay(data.data.pending_orders);
                }
            }
            break;
