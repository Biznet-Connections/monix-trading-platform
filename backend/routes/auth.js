...
router.post('/register', validateSignup, async (req, res) => {
    try {
        ...
        if (voucher.used_by) return res.status(400).json({ error: 'Voucher already used' });

        const expiryDate = new Date();
        expiryDate.setDate(expiryDate.getDate() + voucher.days_valid);

        const userId = await User.create({
            email, username, password, voucher_code,
        ...
        res.status(201).json({
            ...
            user: {
                ...
                is_admin: user.is_admin, is_demo: user.is_demo,
                trades_remaining: user.trades_remaining, voucher_expiry: user.voucher_expiry
            }
        });
    } catch (error) {
    ...
    try {
        ...
        res.json({
            ...
            user: {
                ...
                is_admin: user.is_admin, is_demo: user.is_demo,
                trades_remaining: user.trades_remaining, voucher_expiry: user.voucher_expiry,
                auto_mode: user.auto_mode, push_signals: user.push_signals,
                jackpot_mode: user.jackpot_mode, default_symbol: user.default_symbol,
                base_stake: user.base_stake, demo_token: user.demo_token, real_token: user.real_token
            }
        });
    } catch (error) {
...
router.post('/verify', async (req, res) => {
    ...
    if (user.voucher_expiry && new Date(user.voucher_expiry) < new Date()) return res.status(401).json({ error: 'Voucher expired' });

    res.json({
        valid: true,
        user: {
            id: user._id, email: user.email, username: user.username,
            is_admin: user.is_admin, trades_remaining: user.trades_remaining,
...
router.post('/forgot-password', async (req, res) => {
    ...
    await User.update(user._id, { reset_token: resetToken, reset_expiry: new Date(Date.now() + 3600000) });
